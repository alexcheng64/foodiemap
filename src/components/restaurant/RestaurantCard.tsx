...
export function RestaurantCard({ restaurant, onClick }: RestaurantCardProps) {
  ...
  const getAverageSpending = (level?: number) => {
          ...
          <div className="flex items-center gap-4 mt-2">
            {restaurant.rating && (
              <div className="flex items-center gap-1 text-sm">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <svg
                      key={star}
                      className={`w-4 h-4 ${
                        star <= Math.round(restaurant.rating!) ? 'text-yellow-400' : 'text-gray-300'
                      }`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                ...
                <span className="font-medium text-gray-700">{restaurant.rating}/5</span>
